...
**Phạm vi đã triển khai:** Nền tảng A–B, backend C0–C3, khung frontend D1 và phần đầu D2.
...
API giỏ hàng C3 đã triển khai trong `cart_service.py`, với transaction tuần tự theo buyer để bảo vệ quy tắc một giỏ một shop. Quy tắc chi tiết nằm trong [`BUSINESS_RULES.md`](BUSINESS_RULES.md#giỏ-hàng-c3). Frontend chưa nối API này: nút thêm vào giỏ vẫn bị khóa, phần giao diện giỏ hàng thuộc bước D2 tiếp theo. Danh sách review, các trang checkout/đơn hàng cùng giao diện nghiệp vụ shop và admin vẫn là **Planned**. Rating trung bình trên catalog và chi tiết lấy từ C2; dashboard shop/admin hiện vẫn là màn hình khung.
