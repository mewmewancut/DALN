...
Frontend D1–D3 có test gắn token, xử lý `401`, điều hướng theo vai trò, form auth, catalog, toàn bộ luồng buyer từ giỏ hàng tới review và toàn bộ trang quản lý của chủ shop.
...
| Frontend | Vitest + jsdom | Axios token/`401`, route theo vai trò, auth/catalog, đổi shop trong giỏ, sửa/xóa giỏ, checkout, lọc/hủy đơn, review và các trang SHOP_OWNER D3 |
...
- Frontend SHOP_OWNER D3: chủ shop chưa có shop chỉ thấy form tạo shop (không gọi API quản lý), tạo thành công thì lưu `shop_id` vào phiên và mở sidebar, tạo lỗi thì giữ form và hiện lỗi API; dashboard gọi `overview`/`revenue-by-day` với đúng khoảng 30 ngày mặc định, gọi lại khi đổi ngày, hiện 4 chỉ số, `—` khi `cancel_rate`/`aov` là `null`, badge số cảnh báo, lỗi `400` khi khoảng ngày sai và biểu đồ điền 0 cho ngày thiếu kể cả khi qua tháng; trang sản phẩm hiện cả sản phẩm đã ẩn, lọc `is_active`, bật/tắt qua `PUT /products/{id}`, tạo sản phẩm gửi mọi biến thể trong một request và giữ dialog khi lỗi, sửa thông tin, giá/trạng thái variant và thêm variant; trang đơn hàng chỉ hiện nút đúng transition của shop cho từng trạng thái, gửi `PATCH` đúng body, hủy kèm lý do trong `note`, lọc theo trạng thái và hiện lỗi backend; tồn kho tô đỏ dòng `is_low`, chặn ngưỡng không hợp lệ, cập nhật dòng theo response và giữ giá trị khi lỗi; cảnh báo hiện danh sách hoặc trạng thái rỗng; nhà cung cấp thêm/sửa/ngừng hợp tác/khôi phục đúng endpoint; phiếu nhập chỉ cho chọn nhà cung cấp đang hợp tác, chặn biến thể trùng, gửi đúng body, hiện nút theo trạng thái và link sang tồn kho sau khi nhận hàng.

Toàn bộ backend Planning C0–C10 và frontend D1–D3 đã có test. Phần còn lại là frontend ADMIN (D4) và data platform (E).
