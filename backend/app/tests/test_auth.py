...
def test_login_token_me_and_invalid_credentials(
    ...
    assert client.post(
        "/auth/login", json={"email": owner.email, "password": "wrong"}
    ).status_code == 401
    response = client.post(
        "/auth/login", json={"email": owner.email, "password": "Secret@123"}
    )
...
def test_locked_user_and_bad_tokens(db_session: Session, client: TestClient) -> None:
    ...
    assert client.post(
        "/auth/login", json={"email": user.email, "password": "Secret@123"}
    ).status_code == 403
    ...
    assert client.get(
        "/auth/me", headers={"Authorization": f"Bearer {active_token}"}
    ).status_code == 403
